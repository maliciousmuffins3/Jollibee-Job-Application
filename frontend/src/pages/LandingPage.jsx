...
const LandingPage = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        // Make sure the environment variable is prefixed with VITE_
        const res = await fetch(import.meta.env.VITE_SERVER_URL + "/available_position/show-positions");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://jollibee-job-application.onrender.com" + "/applicants/add-applicant", {
        method: "POST",
        body: form,
      });
      const data = await res.json();
